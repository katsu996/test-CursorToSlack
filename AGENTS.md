...
| `docs/table/pages_ui_config.json` | Pages トップの列幅・既定表示・`index_table`（`leading_table_columns`・列順・任意の `trailing_table_columns`・ラベル・IR/Chart URL・`column_visible_defaults.ir` / `chart` 等。`build_pages_table.py` が `meta.pages_ui` に埋め込み。`//` コメント可） |
