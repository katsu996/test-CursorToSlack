...
**一覧の URL クエリ（状態の共有）:** 並び替え・キーワード・難易度表チェック・列の表示・ページ送り・ツールバー開閉をクエリに反映し、同じ表示をリンクで再現できます（アドレスは `history.replaceState` で更新されます）。例: `?q=foo`、`?s1=table:custom_level&d1=asc`（優先順位 1 の既定は **独自レベル**の昇順。`s2`/`d2`、`s3`/`d3` も同様）、`?src=sl,st`（表示する略称のみ。クエリに **`src` を付けたときだけ** 表チェックを上書き）、`?tcols=title,level&dcols=minbpm`（表示する列キーのカンマ区切り。付けたときだけ上書き）、`?ir=0`（IR 3 列を隠す）、`?ch=0`（Chart を隠す）、`?pg=2`（1 ページあたり最大 150 行）、`?tb=1`（ツールバーを開いた状態）。既定と同じ値はクエリから省かれます。
