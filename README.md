...
**Pages のトップ:** `index.html` のフッターに、上記と同じパスの **`filtered_header.json` を開くリンク**があります（新しいタブで JSON が表示されます）。Table URL の確認や共有に使えます。一覧表では **独自レベル（`custom_level`）** を **最左の単独グループ**とし、その右に **元難易度表由来の列**、**楽曲情報（DB）**、**IR**（LR2IR / MinIR / Mocha）、**Chart** の順です。リンク先 URL は **`pages_ui_config.json` の `index_table`**（`browser_rows.json` の `meta.pages_ui` に埋め込み）で変更できます。既定では難易度表行の **`md5`**（LR2IR・Chart）および **`sha256`**（MinIR・Mocha）から外部ビューアへのリンクです。

**一覧の URL クエリ（状態の共有）:** 並び替え・キーワード・難易度表チェック・**独自レベル**・列の表示・ページ送り・ツールバー開閉をクエリに反映し、同じ表示をリンクで再現できます（アドレスは `history.replaceState` で更新されます）。例: `?q=foo`、`?s1=table:custom_level&d1=asc`（優先順位 1 の既定は **独自レベル**の昇順。`s2`/`d2`、`s3`/`d3` も同様）、`?src=sl,st`（表示する略称のみ。クエリに **`src` を付けたときだけ** 表チェックを上書き）、`?cl=5,12`（表示する **独自レベル**をカンマ区切り。記号や空白を含む値は `encodeURIComponent` した断片を並べる。**`cl=` のみ**＝チェックをすべて外した状態＝行を表示しない。クエリに **`cl` を付けたときだけ** 独自レベル絞り込みを上書き）、`?tcols=title,level&dcols=minbpm`（表示する列キーのカンマ区切り。付けたときだけ上書き）、`?ir=0`（IR 3 列を隠す）、`?ch=0`（Chart を隠す）、`?pg=2`（2 ページ目）、`?ps=100`（1 ページの行数。`50` / `100` / `150` / `200` / `300` / `500` のいずれか。既定 150 のときは省略）、`?tb=1`（ツールバーを開いた状態）。既定と同じ値はクエリから省かれます。
